...
let rec loop lexbuf checkpoint =
  ...
  let string_of_terminal : type a. a I.terminal -> string option =
   fun terminal ->
    ...
    | T_RSQUARE -> Some "right-bracket"
    | T_RPAREN -> Some "right-parenthesis"
    | T_RCURLY -> Some "right-brace"
    | T_MINUSGREATER -> Some "arrow"
    | T_MINUS -> Some "minus symbol"
    | T_LSQUARE -> Some "left-bracket"
    | T_LPAREN -> Some "left-parenthesis"
    | T_LET -> Some "keyword \"let\""
    | T_LCURLY -> Some "left-brace"
    | T_LABEL -> Some "label"
  ...
  let string_of_nonterminal : type a. a I.nonterminal -> string option = function
    ...
    | N_row | N_fixed_row | N_extended_row -> Some "a row"
    | N_record | N_fixed_record | N_extended_record -> Some "a record"
    | N_pat_case -> Some "a patterned case"
    | N_list_dec_ | N_file_exn -> Some "a list of commands"
    | N_default_case -> Some "a default case"
    | N_dec_exn | N_dec -> Some "a command"
    | N_cases -> Some "a list of cases"
  in
